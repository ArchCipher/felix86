...
bool should_log_strace(const char* name, ssize_t result) {
    ...
    static std::unordered_set<std::string> syscall_list = {};
    static std::unordered_set<std::string> syscall_errors_list = {};
    std::vector<std::string> split;
    if (syscall_list.empty() && !g_config.strace.empty()) {
        split = split_string(g_config.strace, ',');
        for (const auto& s : split) {
            syscall_list.insert(s);
        }
    }
    if (syscall_errors_list.empty() && !g_config.strace_errors.empty()) {
        split = split_string(g_config.strace_errors, ',');
        for (const auto& s : split) {
            syscall_errors_list.insert(s);
        }
    }
    return (syscall_list.contains(name) || (syscall_errors_list.contains(name) && result < 0));
}
